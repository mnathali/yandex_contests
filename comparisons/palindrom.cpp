...
int binary_search_palindrom(int i, int j, hash_type &v, hash_type &v_reverse, unsigned p)
{
    ...
    int r = size - j - 1 > i ? i : size - j - 1, l = 1;
    if (!r || (v[j+1].first + v_reverse[size - j - 2].first * v_reverse[j - i + 2].second) % p
            != (v_reverse[size - i].first + v[i-1].first * v[j - i + 2].second) % p)
            return cnt;
    else if ((v[j+r].first + v_reverse[size - j - r - 1].first * v_reverse[j - i + 2 * r].second) % p
            == (v_reverse[size - i + r - 1].first + v[i-r].first * v[j - i + 2 * r].second) % p)
                return cnt + r;
    while (r-1 > l)
    {
        if ((v[j + l +(r-l)/2].first + v_reverse[size - j - l -(r-l)/2 - 1].first * v_reverse[j - i + 2 * (l + (r-l)/2)].second) % p
            == (v_reverse[size - i + l + (r-l)/2 - 1].first + v[i-l-(r-l)/2].first * v[j - i + 2 * (l + (r-l)/2)].second) % p)
            l += (r - l) / 2;
        else
            r = l + (r - l) / 2; 
    }
